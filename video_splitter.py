...
class VideoSplitter:
    ...
    def _generate_resolve_project(self, source_video: str):
        """Generate DaVinci Resolve project files for easy import"""
        # Create Resolve project XML (simplified version)
        root = ET.Element("ResolveProject")
        root.set("version", "1.0")

        # Project info
        project_info = ET.SubElement(root, "ProjectInfo")
        ET.SubElement(project_info, "Name").text = self.project_name
        ET.SubElement(project_info, "Created").text = datetime.now().isoformat()

        # Timeline
        timeline = ET.SubElement(root, "Timeline")
        timeline.set("name", f"{self.project_name}_Timeline")

        # Add clips to timeline
        for clip_info in self.clip_metadata:
            clip_element = ET.SubElement(timeline, "Clip")
            ET.SubElement(clip_element, "FilePath").text = clip_info['filepath']
            ET.SubElement(clip_element, "FileName").text = clip_info['filename']
            ET.SubElement(clip_element, "StartTime").text = str(clip_info['start_time'])
            ET.SubElement(clip_element, "Duration").text = str(clip_info['duration'])
            ET.SubElement(clip_element, "ClipNumber").text = str(clip_info['clip_number'])

        # Save XML
        xml_file = self.resolve_dir / f"{self.project_name}.drp"
        tree = ET.ElementTree(root)
        tree.write(xml_file, encoding='utf-8', xml_declaration=True)

        # Create batch import script for Resolve
        batch_script = f"""-- DaVinci Resolve Batch Import Script
-- Generated by LTW Video Splitter
...
if not timeline then
    timeline = project:CreateTimeline("{self.project_name}_Timeline")
end
...
"""
        ...
        for clip_info in self.clip_metadata:
            batch_script += f'    "{clip_info["filepath"]}",\n'

        batch_script += """}

for _, clipPath in ipairs(clips) do
    local clip = resolve:ImportMedia(clipPath)
    if clip then
        timeline:AppendClip(clip)
    end
end

print("Batch import completed for {self.project_name}")
"""
        ...
        # Create README for Resolve users
        readme_content = f"""# DaVinci Resolve Project: {self.project_name}

## Overview
This project contains {len(self.clip_metadata)} video clips split from "{source_video}".

## Files
- `{self.project_name}.drp` - DaVinci Resolve project file
- `{self.project_name}_import.lua` - Lua script for batch import
- `../metadata/{self.project_name}_metadata.json` - Detailed clip information

## How to Import

### Option 1: Use the Lua Script (Recommended)
1. Open DaVinci Resolve
2. Go to Workspace > Scripts > Run Script
3. Select `{self.project_name}_import.lua`
4. The script will create a new timeline and import all clips

### Option 2: Manual Import
1. Open DaVinci Resolve
2. Import all clips from the `../clips/` folder
